...
const ohMy = () => {
    axios.get(`${baseURL}/animals`).then(results => {
        for(let i = 0; i < results.data.length; i++){
            let p = document.createElement('p')
            p.textContent = results.data[i]
            document.body.appendChild(p)
            console.log(results.data[i])
        }
    }).catch((err) => console.log({err, message: "Problem running function."}))
}
...
function myChoice(){
    axios.get(`${baseURL}/query-test/?name=brooklyn age=100`).then(results => {
        console.log(results.data)
    }).catch((err) => console.log({err, message: "Problem with myChoice function."}))
}
document.querySelector('#query-button').addEventListener('click', myChoice)
...
function createFood(e){
    ...
    axios.post(`${baseURL}/food`, body).then(results => {
        console.log(results.data)
        document.getElementById(`form-p`).textContent = results.data
    }).catch((err) => console.log({err, message: "Problem with createFood"}))
}
